...
MODULE_NAME = "Regression - Facebook Reply Workflow on PreProd"
today_str = time.strftime("%d-%b-%Y").upper()

RAW_TEST_PATHS = [
    "tests/selenium/PreProd/facebook-preprod/test_fb_regression.py"
    # "tests/selenium/PreProd/facebook-preprod/test_reply_and_assign.py",
    # "tests/selenium/PreProd/facebook-preprod/test_reply_and_await.py",
    # "tests/selenium/PreProd/facebook-preprod/test_reply_and_onHold.py",
    # "tests/selenium/PreProd/facebook-preprod/test_reply_and_escalate.py",
    # "tests/selenium/PreProd/facebook-preprod/test_csd_approve.py",
    # "tests/selenium/PreProd/facebook-preprod/test_reply_and_close.py",
    # "tests/selenium/PreProd/facebook-preprod/test_otherActions_openTicket.py"
    # "tests/selenium/PreProd/facebook/test_otherActions_closedTicket.py"
]
...
def send_email_html(report_dir: Path, results_dir: Path, duration: float, exit_code: int, timing: TimingCollector):
    ...
    environment = os.getenv("REPORT_ENVIRONMENT") or os.getenv("ENVIRONMENT") or "PreProd"
    config_rows_html = (
    ...
    html = build_enterprise_email_html(
        ...
        suite_subtitle="Facebook • PreProd Regression",
    ...
    subject = f"Facebook (PreProd) Suite {today_str}"
    try:
...
def run_suite():
    ...
    print("Debug: SELENIUM_SCRIPTS resolved paths:")
    for p in SELENIUM_SCRIPTS:
        print(f"  {p} -> {'OK' if os.path.exists(p) else 'MISS'}")
