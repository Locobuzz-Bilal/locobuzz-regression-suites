# Email Regression with all reply workflows and other actions
import os
import sys
import time
import glob
...
MODULE_NAME = "Regression - Email Workflow"
today_str = time.strftime("%d-%b-%Y").upper()

RAW_TEST_PATHS = [
    "tests/selenium/email/test_email_reply_assign.py",
    "tests/selenium/email/test_email_reply_await.py",
    "tests/selenium/email/test_email_reply_onhold.py",
    "tests/selenium/email/test_email_reply_escalate.py",
    "tests/selenium/email/test_email_csd_approve.py",
    "tests/selenium/email/test_email_reply_close.py",
    "tests/selenium/email/test_email_other_actions_open_ticket.py",
]
...
def send_email_html(report_dir: Path, results_dir: Path, duration: float, exit_code: int, timing: TimingCollector):
    ...
    html = build_enterprise_email_html(
        ...
        suite_subtitle="Email • Automated Regression",
    ...
    subject = f"Email Suite {today_str}"
    try:
...
def run_suite():
    ...
    # Check if --reruns was passed via command line
    reruns_from_cli = None
    reruns_delay_from_cli = None
    
    for i, arg in enumerate(sys.argv):
        if arg == "--reruns" and i + 1 < len(sys.argv):
            reruns_from_cli = sys.argv[i + 1]
        elif arg == "--reruns-delay" and i + 1 < len(sys.argv):
            reruns_delay_from_cli = sys.argv[i + 1]
    
    # Use CLI values if provided, otherwise use defaults
    reruns_count = reruns_from_cli if reruns_from_cli is not None else "2"
    reruns_delay = reruns_delay_from_cli if reruns_delay_from_cli is not None else "2"
    
    print(f"Global reruns: {reruns_count} (delay {reruns_delay}s)")

    pytest_args = [
        *SELENIUM_SCRIPTS,
        "-v", "-s", "--tb=short", "--cache-clear",
        "--reruns", reruns_count, "--reruns-delay", reruns_delay,
